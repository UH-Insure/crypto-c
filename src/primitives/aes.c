...
/* ===== AES-128 per FIPS-197 (ECB single-block) ===== */
...
static inline void add_round_key(uint8_t s[16], const uint8_t rk[16]){
...
static inline void inv_mix_columns(uint8_t s[16]){
    for (int c=0;c<4;c++){
        int i = 4*c;
        uint8_t a0=s[i], a1=s[i+1], a2=s[i+2], a3=s[i+3];
        s[i]   = (uint8_t)(mul14(a0) ^ mul11(a1) ^ mul13(a2) ^ mul9(a3));
        s[i+1] = (uint8_t)(mul9(a0)  ^ mul14(a1) ^ mul11(a2) ^ mul13(a3));
        s[i+2] = (uint8_t)(mul13(a0) ^ mul9(a1)  ^ mul14(a2) ^ mul11(a3));
        s[i+3] = (uint8_t)(mul11(a0) ^ mul13(a1) ^ mul9(a2)  ^ mul14(a3));
    }
}
